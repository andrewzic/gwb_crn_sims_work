...
ptasim_inp_template_fn = 'ptasim_input_files/ptasim_all_similar_26_N_template_100r.inp'
...
for y in N:
    ...
    for x in N:
        ...
        if True: #not os.path.exists('ptasim_input_files//{:.2f}_{:.2f}.inp'.format(dP0, dalpha)):
            with open('ptasim_input_files/spinnoise_100r/{:.2f}_{:.2f}.inp'.format(dP0, dalpha), 'w') as ptasim_inp_f:
                ptasim_inp_f.write(ptasim_inp_template_str.format('{:.2f}'.format(dP0), '{:.2f}'.format(dalpha), str_tnoise, str_obs))
                ptasim_inp_f.close()

            with open('psr_noise_vals/100r/{:.2f}_{:.2f}.dat'.format(dP0, dalpha), 'w') as psr_datf:
